...
{
    public class SendChatMessageHandler : IHandleMessages<SendMessageRequest>
    {
        /// <summary>
        ...
        public Task HandleAsync(SendMessageRequest request, IMessageHandlerContext context)
        {
            ChatServiceDatabase db = ChatServiceDatabase.getInstance();

            ServiceBusResponse response = db.SendMessage(request);
