...
namespace AuthenticationService.Communication
{
    ...
    partial class ClientConnection
    {
        ...
        private ServiceBusResponse reviewRequest(ReviewServiceRequest request)
        {
            switch (request.requestType)
            {
                case (ReviewRequest.GetReviews):
                    return GetReviews((GetCompanyReviewsRequest)request);
                case (ReviewRequest.SaveReview):
                    return SaveReviews((SaveCompanyReviewRequest)request);
                default:
        ...
        private ServiceBusResponse GetReviews(GetCompanyReviewsRequest request)
        {
            ...
            sendOptions.SetDestination("Review Service");
        ...
        private ServiceBusResponse SaveReviews(SaveCompanyReviewRequest request)
        {
            if (authenticated == false)
            {
                return new ServiceBusResponse(false, "Error: You must be logged in to use the echo reverse functionality.");
